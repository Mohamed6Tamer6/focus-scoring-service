...
class EnhancedFocusDetector:
    ...
    def get_report(self):
        ...
        present_time = total_time - self.absence_time
        effective_focus = 0.0
        if present_time > 0:
            effective_focus = (self.focus_time / present_time) * 100
        ...
        report = {
            ...
            'effective_focus_rate': effective_focus,
...
class FocusProcessor:
    """
    High-level processor wrapping detector + MediaPipe.
    Designed for WebSocket per-frame processing.
    """
    def __init__(self, focus_zone='normal'):
        self.detector = EnhancedFocusDetector(debug_mode=True)
        self.detector.set_focus_zone(focus_zone)
    ...
    def set_focus_zone(self, zone_name):
        self.detector.set_focus_zone(zone_name)

    def process_frame(self, frame_bytes: bytes) -> dict:
        """Process a single frame (JPEG bytes) and return focus analysis."""
        nparr = np.frombuffer(frame_bytes, np.uint8)
        frame = cv.imdecode(nparr, cv.IMREAD_COLOR)

        if frame is None:
            return {"error": "Failed to decode frame", "face_detected": False}
        ...
        face_present = result.multi_face_landmarks is not None
        self.detector.handle_face_absence(face_present, current_time)

        if face_present:
            landmarks = result.multi_face_landmarks[0].landmark

            # Head pose, ear, blink, focus
            head_pose, rotation_vec, translation_vec = \
                self.detector.head_pose_estimator.estimate_pose(landmarks, frame.shape)
            
            left_ear = self.detector.calculate_ear([33, 160, 158, 133, 153, 144], landmarks, frame.shape)
            right_ear = self.detector.calculate_ear([362, 385, 387, 263, 373, 380], landmarks, frame.shape)
            
            focus_score, components = self.detector.calculate_focus_score(
                head_pose, left_ear, right_ear
            )
            
            is_focused, smoothed_score = self.detector.update_focus_state(
                focus_score, current_time
            )

            total_time = self.detector.total_seconds_tracked
            focus_pct = (self.detector.focus_time / max(total_time, 0.1)) * 100

            # Return all 468 facemesh landmarks for detailed rendering
            facemesh = [{"x": l.x, "y": l.y} for l in landmarks]

            # Pose line (nose tip to projected point)
            # nose_tip = landmarks[1], let's project an axis Z
            nose_tip_x = int(landmarks[1].x * frame.shape[1])
            nose_tip_y = int(landmarks[1].y * frame.shape[0])

            # For simplicity, just return the raw data and let JS draw.
            
            return {
                "face_detected": True,
                "is_focused": bool(is_focused),
                "focus_score": round(float(smoothed_score), 3),
                "pitch": round(float(components['angles']['pitch']), 1),
                "yaw": round(float(components['angles']['yaw']), 1),
                "roll": round(float(components['angles']['roll']), 1),
                "total_blinks": self.detector.total_blinks,
                "focus_time": round(self.detector.focus_time, 1),
                "unfocus_time": round(self.detector.unfocus_time, 1),
                "absence_time": round(self.detector.absence_time, 1),
                "total_time": round(total_time, 1),
                "focus_percentage": round(focus_pct, 1),
                "facemesh": facemesh,
                "unfocus_events": len(self.detector.unfocused_periods),
                "absence_events": len(self.detector.absence_periods),
            ...
            return {
                ...
                "is_focused": False,
                "focus_score": 0,
                "total_time": round(self.detector.total_seconds_tracked, 1),
                "focus_time": round(self.detector.focus_time, 1),
                "unfocus_time": round(self.detector.unfocus_time, 1),
                "absence_time": round(self.detector.absence_time, 1),
                "total_blinks": self.detector.total_blinks,
                "focus_percentage": round((self.detector.focus_time / max(self.detector.total_seconds_tracked, 0.1)) * 100, 1),
                "unfocus_events": len(self.detector.unfocused_periods),
                "absence_events": len(self.detector.absence_periods),
            }

    def stop(self) -> dict | None:
        """Finalize session and return report."""
        self.detector.finalize_session()
